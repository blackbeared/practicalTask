...
///
/// A [RowX] may only contain [ColX] children.
///
class RowX extends StatelessWidget {
  ...
  ///
  /// List of the children of type [ColX]
  ///
  final List<ColX> children;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        List<ColX> _children = List.from(children);
        _children.sort(
...
///
class ColX extends StatelessWidget {
  ColX({
    required this.child,
  ...
  /// The child
  ///
  final Function(double width) child;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        var columns = 12 / flexRatio;

        if (index == null) {
          leftPadding = _gutterSize / 2;
          rightPadding = _gutterSize / 2;
        } else if (columns == 1) {
          leftPadding = _gutterSize / 2;
          rightPadding = _gutterSize / 2;
        } else {
          ...
          leftPadding =
              currentIndex % columns == 1 ? _gutterSize / 2 : _gutterSize / 4;
          rightPadding =
              currentIndex % columns == 0 ? _gutterSize / 2 : _gutterSize / 4;
        }
        ...
        Widget widget = Container(
          ...
          child: Padding(
            ...
            child: child(flexRatio * constraints.maxWidth * _oneColumnRatio),
          ),
        );
...
/// defined in [sizes]
///
class VisibilityX extends StatelessWidget {
  VisibilityX({
    required this.child,
    String sizes = "",
  }) : this.sizes = sizes.trim() {
